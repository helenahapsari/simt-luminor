...
$columns = ['NO', 'NAMA', 'NIP', 'TANGGAL MASUK', 'JAM MASUK', 'FOTO MASUK', 'TANGGAL KELUAR', 'JAM KELUAR', 'FOTO PULANG', 'TOTAL JAM KERJA', 'STATUS / TERLAMBAT'];
...
while($data = mysqli_fetch_array($result)){
    ...
    if ($jam_keluar_raw !== '00:00:00' && !empty($jam_keluar_raw)) {
        ...
        $val_status_telat = (!$is_telat) ? "On Time" : "Terlambat ({$jam_t}j {$min_t}m)";
    } else {
        if ($tanggal_raw < $hari_ini) {
            $val_total_jam = "Tidak presensi pulang";
            $val_status_telat = "ALPA";
    ...
    $sheet->setCellValue('K'.$row, $val_status_telat); 
...
// --- BAGIAN PENAMAAN FILE OTOMATIS ---
$tgl_file_dari = date('d-m-Y', strtotime($tanggal_dari));
$tgl_file_sampai = date('d-m-Y', strtotime($tanggal_sampai));

if ($tanggal_dari == $tanggal_sampai) {
    $nama_file = "Rekap_Presensi_Harian_" . $tgl_file_dari . ".xlsx";
} else {
    $nama_file = "Rekap_Presensi_" . $tgl_file_dari . "_sd_" . $tgl_file_sampai . ".xlsx";
}
